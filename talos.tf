...
data "talos_machine_configuration" "controlplane" {
  ...
  config_patches = [
    templatefile("${path.module}/config/control-plane.yml", {}),
  ]
}
...
data "talos_machine_configuration" "worker" {
  ...
  machine_secrets  = talos_machine_secrets.this.machine_secrets
  config_patches = [
    templatefile("${path.module}/config/worker.yml", {}),
  ]
}
...
resource "talos_machine_configuration_apply" "controlplane" {
  ...
  config_patches = [
    templatefile("${path.module}/config/common.yml", {
...
resource "local_file" "talosconfig" {
  ...
  filename        = "${path.module}/config/talosconfig.dec.yml"
...
resource "local_file" "kubeconfig" {
  ...
  filename        = "${path.module}/config/kubeconfig.dec.yml"
  file_permission = "0644"
}
